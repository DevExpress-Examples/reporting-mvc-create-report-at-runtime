...
namespace E4714.Services {
    public class CustomReportProvider : DevExpress.XtraReports.Services.IReportProvider {
        ...
        private object CreateDataSource() {
            string dbFilePath = HttpContext.Current.Server.MapPath(@"/App_Data/nwind.mdb");
            var connectionParameters = new Access97ConnectionParameters(dbFilePath, "", "");
            var ds = new SqlDataSource(connectionParameters);
            SelectQuery query = SelectQueryFluentBuilder.AddTable("Categories").SelectAllColumns().Build("Categories");
